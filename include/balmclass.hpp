...
static void down_sampling_voxel(PL_VEC& pl_feat, double voxel_size)
{
    unordered_map<VOXEL_LOC, M_POINT> feat_map;
    uint plsize = pl_feat.size();
...
static void
plvec_trans_func(vector<Eigen::Vector3d>& orig, vector<Eigen::Vector3d>& tran, Eigen::Matrix3d R, Eigen::Vector3d t)
{
...
template <typename T>
static void pub_func(T& pl, ros::Publisher& pub, const ros::Time& current_time)
{
    pl.height = 1;
    pl.width = pl.size();
...
template <typename T>
static void pub_func(T& pl, ros::Publisher& pub, const std_msgs::Header header)
{
    pl.height = 1;
    pl.width = pl.size();
    sensor_msgs::PointCloud2 output;
...
static void rosmsg2ptype(const sensor_msgs::PointCloud2& pl_msg, pcl::PointCloud<PointType>& plt)
{
...
static const double one_three = (1.0 / 3.0);
static double feat_eigen_limit[2] = {3*3, 2*2};
static double opt_feat_eigen_limit[2] = {4*4, 3*3};

// LM optimizer for map-refine
class LM_SLWD_VOXEL
{
public:
    int slwd_size, filternum, thd_num, jacobian_length;
    ...
    LM_SLWD_VOXEL(int ss, int fn, int thnum) : slwd_size(ss), filternum(fn), thd_num(thnum)
    {
        ...
        jacobian_length = 6 * ss;
        corn_less = 0.1;
        map_refine_flag = 0;
    }

    void set_window_size(int ss)
    {
        slwd_size = ss;
    ...
    void set_eigen_thres(std::vector<double> value)
    {
        assert(value.size() == 6);
                        ...
                        Eigen::VectorXd& JacT, double& residual)
    {
        ...
        for (int j = 0; j < jacobian_length; j += 6)
        {
            for (int i = j + 6; i < jacobian_length; i += 6)
            {
    ...
    // LM process
    void damping_iter(Eigen::Matrix<double, 6, 6>* out_V_f = nullptr, double* degenerate_m = nullptr)
    {
        my_mutex.lock();
        map_refine_flag = 1;
        my_mutex.unlock();
        ...
        double u = 0.01, v = 2;
        Eigen::MatrixXd D(jacobian_length, jacobian_length), Hess(jacobian_length, jacobian_length);
        Eigen::VectorXd JacT(jacobian_length), dxi(jacobian_length);

        Eigen::MatrixXd Hess2(jacobian_length, jacobian_length);
        Eigen::VectorXd JacT2(jacobian_length);

        D.setIdentity();
        double residual1, residual2, q;
        bool is_calc_hess = true;

        cv::Mat matA(jacobian_length, jacobian_length, CV_64F, cv::Scalar::all(0));
        cv::Mat matB(jacobian_length, 1, CV_64F, cv::Scalar::all(0));
        cv::Mat matX(jacobian_length, 1, CV_64F, cv::Scalar::all(0));


        vector<Eigen::Vector3d> t_poses_predict(slwd_size);
        for (int state_idx = 0; state_idx < slwd_size; state_idx++)
        {
            t_poses_predict[state_idx] = t_poses[state_idx];
        }

        vector<bool> is_degenerate(slwd_size, false);

        vector<Eigen::Matrix<double, 6, 6>> remap(slwd_size);

        // vector<Eigen::Matrix<double, 6, 6>> remap_u(slwd_size);
        ...
        for (int iter = 0; iter < iter_max; iter++)
        {
            ...
            for (int i = 0; i < jacobian_length; i++)
            {
                matB.at<double>(i, 0) = -JacT(i, 0);
                for (int j = 0; j < jacobian_length; j++)
                {
                    matA.at<double>(i, j) = Hess2(i, j);
                }
            }
            ...
            for (int i = 0; i < jacobian_length; i++)
            {
                dxi(i, 0) = matX.at<double>(i, 0);
            }

            // the first iteration, do degenerate analysis
            if (iter == 0)
            {
                ROS_DEBUG("[BALM] ============================== degenerate analysis =================================");

                for (int state_idx = 0; state_idx < slwd_size; state_idx++)
                {
                    Eigen::Matrix<double, 6, 6> AtA = Hess.block<6, 6>(6 * (state_idx), 6 * (state_idx));
                    Eigen::SelfAdjointEigenSolver<Eigen::Matrix<double, 6, 6>> saes(AtA);
                    Eigen::Matrix<double, 1, 6> values = saes.eigenvalues().real();
                    Eigen::Matrix<double, 6, 6> V_f = saes.eigenvectors().real().transpose(); // eq.(17)

                    Eigen::Matrix<double, 6, 6>  V_u = V_f;

                    // 从小到大排序
                    int m = 0;
                    while (m < 6)
                    {
                        if (values[m] < eigen_threshold[m])
                        {
                            V_u.row(m).setZero();
...
//                    remap[state_idx] = V_f.inverse() * V_u;
                    ...
                    if (state_idx == slwd_size - 1)
                    {
                        Eigen::IOFormat kCleanFmt{4, 0, ", ", "\n", "dir: [", "]"};
                        ROS_DEBUG_STREAM("[BALM] eigen_value = \n" << values.head<6>() << "\nmatV = \n" << V_f.format(kCleanFmt));

                        if (out_V_f ) *out_V_f = V_f;
                        if (degenerate_m) *degenerate_m = m;
                    }
                }
...
//                if ((t_poses_predict[state_idx] - t_poses_temp[state_idx]).norm() > 0.2)
//                {
//                    t_poses_temp[state_idx] = t_poses_predict[state_idx];
//                }
...
public:
    ...
    Eigen::Vector3f random_color = 0.5 * (Eigen::Vector3f::Random() + Eigen::Vector3f::Ones());
