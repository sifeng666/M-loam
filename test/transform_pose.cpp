...
string path = "/home/ziv/mloam/5g/loop/compare/";
//vector<string> methods{"aloam", "no_skip_with_info", "no_skip_with_no_info", "skip"};
vector<string> methods{"aloam", "ours"};
...
//    }
    // mapping
    for (auto m : methods) {
        string filename = path + m + ".txt";
        string filename_out = path + m + "_out.txt";
        f.open(filename);
        f_out.open(filename_out);
        Eigen::Isometry3d pose(Eigen::Matrix4d::Identity());
